...
Este laboratorio junta las dos ideas centrales del capítulo. Con el primer deslizador cambias el **tamaño de muestra** y ves cómo se angosta la distribución de estimaciones. Con el segundo introduces un **sesgo** en el método de selección y ves algo incómodo: por más que aumentes la muestra, las estimaciones se apretujan alrededor del valor **equivocado**.
...
::: {.callout-note title="Interpreta"}
Con **sesgo en cero**, aumentar $n$ angosta el histograma alrededor de la línea roja: más datos, más precisión. Ahora sube el sesgo a 0.10 y vuelve a subir $n$ hasta 300: el histograma se vuelve **angosto pero centrado en el lugar equivocado**. Ahí está, en una imagen, la lección del capítulo: el tamaño de muestra reduce el **error de muestreo**, nunca el **sesgo**. Una estimación sesgada con muestra grande es peligrosa justamente porque *parece* precisa.
:::
...
Este campo tiene 96 lagartijas de tres especies, cada una ocupando **su propia zona** del terreno: la A la mitad izquierda (48 individuos, 50 %), la B la franja central (32, 33 %) y la C la orilla derecha (16, 17 %). Elige un método, cambia el tamaño de muestra y fíjate en dos cosas: **dónde** caen los individuos seleccionados y qué tan bien la **composición de la muestra** reproduce la de la población.
...
Con **conglomerados**, se eligen **parcelas completas** al azar (los recuadros punteados) y se mide todo lo que hay dentro. Es mucho más barato en campo —te desplazas a dos parcelas en vez de recorrer las 96 posiciones—, pero como cada parcela cae casi por completo dentro de una zona, la muestra puede quedar compuesta **casi de una sola especie**. Vuelve a elegir este método varias veces y observa saltar las barras.
...
| Grupos claros que quieres representar bien | Estratificado |
...
1. **Identifica el método.** Para cada caso, di qué tipo de muestreo se usó:
...
2. **Población y unidad.** En una encuesta de satisfacción a estudiantes de la LCC, identifica la población, la unidad de muestreo y un posible parámetro de interés.

3. **Tipo de sesgo.** Clasifica cada situación como sesgo de selección o de medición y explica:
...
4. **El tamaño no salva.** Explica con tus palabras por qué una encuesta con un millón de respuestas puede ser peor que una de mil bien muestreada.

5. **Simulación en R.** Repite la simulación de proporciones del capítulo pero con muestras de tamaño 150 en lugar de 40. Compara el ancho de los dos histogramas. ¿Qué error disminuyó: el de muestreo o el no de muestreo?

6. **Diseña un muestreo.** Quieres estimar la preferencia por una nueva bebida entre los clientes de una cadena de cafeterías con 100 sucursales en la ciudad. Propón un método de muestreo, justifícalo y señala un sesgo que tu diseño ayude a evitar.

7. **Experimenta con el simulador.** En el laboratorio del @sec-lab-metodos, fija el tamaño de muestra en 24 y vuelve a elegir **cinco veces** el muestreo por conglomerados, anotando cada vez el porcentaje de la especie C en la muestra. Repite con el estratificado. ¿Cuál de los dos da resultados más estables, y por qué? Relaciona tu respuesta con el hecho de que cada especie ocupa su propia zona del campo.
...
**2.** Población: todos los estudiantes inscritos en la LCC. Unidad de muestreo: cada estudiante. Parámetro de interés: por ejemplo, la **proporción** de estudiantes satisfechos con las actividades extracurriculares, o la **media** de una calificación de satisfacción del 1 al 10. *Recuerda: el parámetro se refiere siempre a la población, no a la muestra.*

**3.** (a) **Sesgo de selección**: el lugar donde se aplica la encuesta favorece sistemáticamente a personas de mayores ingresos, que no representan a la población general. (b) **Sesgo de medición**: la redacción de la pregunta induce la respuesta; el instrumento distorsiona lo que se mide. *Truco para distinguirlos: pregúntate si el problema está en **a quién le pregunto** (selección) o en **cómo le pregunto** (medición).*

**4.** Porque el tamaño solo reduce el error de muestreo, mientras que el sesgo es un error **sistemático** que no se promedia hacia cero. Con un método sesgado, un millón de respuestas dan una estimación muy precisa de un número equivocado, y encima la precisión aparente genera una falsa confianza. Mil respuestas bien aleatorizadas estiman el valor correcto con un margen de error honesto. *Puedes verlo tú misma en el laboratorio del @sec-lab-n subiendo el sesgo y luego n.*
