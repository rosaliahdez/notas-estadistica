...
::: {.callout-note title="De dónde vienen estos datos"}
...
![Las medidas del pico. El *culmen* es la cresta superior del pico; en los datos aparece como `bill_length_mm` y `bill_depth_mm`. Ilustración: *Artwork by \@allison_horst*.](figuras/culmen_depth.png){width=58% fig-align="center"}

Definir **cómo** se mide una variable es parte del trabajo estadístico: si dos personas midieran el pico de forma distinta, los datos no serían comparables. Por eso el protocolo especifica el instrumento, su precisión y hasta cuál de las dos aletas se mide.

::: {.callout-note title="Interpretación"}
Antes de calcular nada, responde: en la tabla, ¿qué es un individuo y qué es cada variable? Confundir esto es la fuente número uno de errores. En los datos de **galletas** del curso, por ejemplo, ¿el individuo es una marca o una calificación de un juez? (Es cada *calificación*: una fila por juez y marca.)
:::
...
Nótese que R trató `agrado` como numérica (correcto: es una calificación en escala 1–9) y `marca` como texto. Muchas veces querremos decirle a R que una columna es **categórica**; en R eso se llama *factor*:
...
¿Cuántos intervalos debe tener un histograma? No hay una respuesta única, y la elección **importa**: con muy pocos se pierde el detalle, con demasiados solo se ve ruido. Mueve el deslizador  y se observa cuándo aparecen (o desaparecen) los dos montículos de la masa corporal.
...
2. **Individuos y variables.** Abrir `datos/lagartijas.csv` con `read.csv()`. ¿Qué es un individuo en esa tabla? Lista sus variables y clasifícalas.

3. **Observacional o experimental.** Clasificar cada estudio y justificar en una frase:
...
4. **Barras.** Con los datos de `hongos_sustrato.csv`, hacer un diagrama de barras del número de bolsas por `sustrato`. ¿Está balanceado el experimento?

5. **Histograma y forma.** Graficar el histograma de `flipper_length_mm` de los pingüinos con `binwidth = 5`. Describir centro, dispersión y forma. ¿Se observan indicios de más de un grupo?

6. **Escala de medición (opción múltiple).** El nivel de agrado en una escala "bajo / medio / alto" es una variable de escala: (a) nominal, (b) ordinal, (c) intervalo, (d) razón. Justificar.
