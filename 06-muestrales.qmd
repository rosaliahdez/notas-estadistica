...
Aquí el deslizador vale más que mil palabras. Elige una población **deliberadamente no normal** —sesgada, uniforme o bimodal— y luego aumenta $n$. Observa cómo la distribución de las medias se vuelve acampanada y se angosta, aunque la población de origen no tenga nada de campana.
...
Con $n = 1$ las dos gráficas son iguales: una muestra de tamaño uno *es* un dato individual. Al subir a $n = 5$ la asimetría ya se suaviza; hacia $n = 30$ la distribución de las medias es prácticamente una campana simétrica, **aunque la población de arriba siga siendo sesgada o bimodal**. Nota también que la campana se va **angostando**: es el error estándar $\sigma/\sqrt{n}$ encogiéndose. Prueba con "bimodal", que es el caso más contraintuitivo: la población tiene dos jorobas y las medias forman una sola campana.
:::
...
1. **Parámetro o estadístico.** Clasifica cada cantidad: (a) la masa media de los 344 pingüinos medidos; (b) la proporción de positivos de todas las personas de Sonora; (c) la desviación estándar de la temperatura de las 32 lagartijas urbanas de nuestra base.

2. **Error estándar.** Una población tiene $\sigma = 20$. Calcula el error estándar de la media para $n = 25$, $n = 100$ y $n = 400$. ¿Cuánto hay que aumentar la muestra para reducir el error estándar a la mitad?

3. **¿TCL o no?** ¿En cuáles de estos casos puedes suponer que la media muestral es aproximadamente normal? (a) $n = 50$ de una población muy sesgada; (b) $n = 4$ de una población muy sesgada; (c) $n = 4$ de una población normal.

4. **Simulación.** Repite la simulación de la media muestral con $n = 5$ y con $n = 80$, usando `rexp(n, rate = 1)` como población (exponencial, muy sesgada). Compara los histogramas y las desviaciones estándar.
...
6. **Interpretación.** Explica en dos frases, para alguien que no sabe estadística, por qué una encuesta a 1000 personas puede describir a un país entero.

::: {.callout-tip title="Soluciones y retroalimentación" collapse="true"}
**1.** (a) **Estadístico**: se calculó sobre los pingüinos observados (la muestra). (b) **Parámetro**: describe a toda la población de Sonora. (c) **Estadístico**: se calculó sobre las lagartijas de nuestra base. *Regla rápida: si lo pudiste calcular con tus datos, es un estadístico; si te gustaría conocerlo pero no lo tienes, es un parámetro.*
